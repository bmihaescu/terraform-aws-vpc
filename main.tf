...
resource "aws_route_table_association" "redshift" {
  count = "${var.public_redshift == false && var.create_vpc && length(var.redshift_subnets) > 0 ? length(var.redshift_subnets) : 0}"
...
resource "aws_route_table_association" "redshift_public" {
  count = "${var.public_redshift && var.create_vpc && length(var.redshift_subnets) > 0 ? length(var.redshift_subnets) : 0}"
